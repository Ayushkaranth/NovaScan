...
import { 
  ...
  Loader2, Key, Building2, AlertTriangle
} from "lucide-react";
...
export default function SettingsPage() {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (res.data.length > 0) {
        // Default to the first project for the "Integrations" tab
  ...
  const handleDeleteProject = async (orgId: string, orgName: string) => {
    const confirm = window.confirm(`⚠️ DANGER: Are you sure you want to delete "${orgName}"?\n\nThis action cannot be undone. All scans and history will be lost.`);
    
    if (confirm) {
      try {
        await api.delete(`/organizations/${orgId}`); // Ensure you have this endpoint in backend
        // Remove from local state immediately
        setProjects(projects.filter(p => p._id !== orgId));
        if (currentOrg?._id === orgId) {
          setCurrentOrg(null); // Clear form if current was deleted
        }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {activeTab !== 'danger' && currentOrg && (
          <Button 
            onClick={handleSave} 
            disabled={saving}
            className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-200 rounded-xl px-6"
          >
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'integrations' && (
            <motion.div 
              ...
              className="space-y-8"
            >
              {/* Project Selector for Integrations */}
                ...
                <div className="grid grid-cols-1 gap-4 pl-12">
                   <div className="space-y-1">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          type="password"
                          className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all font-mono"
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-12">
                   <div className="space-y-1">
                      ...
                      <input 
                        type="password"
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-purple-500/20 outline-none transition-all font-mono"
                   ...
                   <div className="space-y-1">
                      ...
                      <input 
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-purple-500/20 outline-none transition-all"
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-12">
                   ...
                   <div className="space-y-1">
                      ...
                      <input 
                        type="password"
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 outline-none transition-all font-mono"
              ...
              {currentOrg ? (
                <div className="space-y-4">
                   <div className="flex justify-between items-center">
                    <h3 className="font-bold text-slate-900">Members of {currentOrg.name}</h3>
                    <span className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-500">{currentOrg.members.length} Users</span>
                   </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {currentOrg.members.map((memberId: string) => (
                      <div key={memberId} className="flex items-center gap-3 p-4 border border-slate-200 rounded-xl">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900">User_{memberId.slice(-6)}</p>
                          <p className="text-xs text-slate-500 font-mono">ID: {memberId}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            >
              <div className="border-l-4 border-red-500 bg-red-50 p-4 rounded-r-xl">
                <h3 className="text-lg font-bold text-red-800 flex items-center gap-2">
                  <ShieldAlert className="w-5 h-5" /> Global Project Management
                </h3>
                <p className="text-sm text-red-700/80 mt-1">
                  You are in the Danger Zone. Actions here (like deletion) are irreversible and affect the entire organization.
                </p>
              </div>
                    ...
                    <div key={proj._id} className="flex items-center justify-between p-5 border border-slate-200 rounded-2xl bg-white hover:border-red-200 transition-all group">
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-1 text-sm text-slate-500">
                          <span className="flex items-center gap-1"><Github className="w-3 h-3" /> {proj.repo_name}</span>
                          <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {proj.members?.length || 0} Members</span>
                        </div>
                      </div>
                      <Button 
                        variant="destructive" 
                        className="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white border border-red-100 transition-all"
