...
export default function DashboardPage() {
  ...
  const [allUsers, setAllUsers] = useState<any[]>([]); // <--- NEW: Store all users to look up names
  ...
  const chartData = useMemo(() => {
    ...
    const trend = [...scans].reverse().map((scan) => {
      const dateStr = scan.timestamp.endsWith("Z") ? scan.timestamp : `${scan.timestamp}Z`;
      return {
        name: `PR #${scan.pr_id}`,
    ...
    const distribution = [
      { name: 'Critical', value: scans.filter(s => s.risk_score > 7).length, color: '#ef4444' },
      { name: 'Warning', value: scans.filter(s => s.risk_score > 4 && s.risk_score <= 7).length, color: '#f97316' },
      { name: 'Safe', value: scans.filter(s => s.risk_score <= 4).length, color: '#10b981' },
    ].filter(d => d.value > 0);
  ...
  const loadDashboardData = async (orgId?: string) => {
    try {
      ...
      // 3. Fetch All Users (To lookup Manager Names) <--- NEW
      try {
        const usersRes = await api.get("/auth/users/list");
        setAllUsers(usersRes.data);
      } catch (err) {
        console.warn("Could not fetch user list (likely permissions), defaulting to IDs");
      }
  ...
  const getManagerName = () => {
    if (!currentOrg?.manager_id) return "Unassigned";
    if (allUsers.length === 0) return `User_${currentOrg.manager_id.slice(-6)}`; // Fallback if list fails
    
    const manager = allUsers.find(u => u._id === currentOrg.manager_id);
            ...
            <div className="bg-white p-6 border border-slate-100 rounded-3xl shadow-sm space-y-6 sticky top-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {/* --- UPDATED MANAGER NAME DISPLAY --- */}
                  <p className="text-base font-bold text-slate-900 mt-0.5 truncate max-w-[150px]" title={getManagerName()}>
                    {getManagerName()}
                  </p>
                  {/* ------------------------------------- */}
...
// --- KEEP YOUR EXISTING HELPER COMPONENTS AS IS (DashboardSkeleton, StatCard, NotificationBell, SetupItem, MasterOnboardingModal) ---
// (Paste them back here from your original file if needed, they are unchanged)
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-slate-50 px-8 py-6 border-b border-slate-100">
           <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2"><Rocket className="text-indigo-600 w-6 h-6" /> Launch New Project</h2>
              <p className="text-sm text-slate-500 mt-1">Configure integrations and team.</p>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-white rounded-full transition-all"><X className="w-5 h-5 text-slate-400" /></button>
          </div>
          <div className="flex items-center gap-2">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex-1"><div className={`h-1.5 rounded-full transition-all duration-500 ${s <= step ? 'bg-indigo-600' : 'bg-slate-200'}`} /><p className={`text-[10px] font-bold uppercase tracking-wider mt-2 ${s <= step ? 'text-indigo-600' : 'text-slate-300'}`}>{s === 1 ? 'Repo' : s === 2 ? 'Integrations' : 'Team'}</p></div>
            ))}
          </div>
        </div>
            ...
            <div className="space-y-5 animate-in fade-in slide-in-from-right-8 duration-300">
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input placeholder="e.g. Backend API" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700 uppercase ml-1">GitHub Owner</label>
                    <input placeholder="Username" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.repo_owner} onChange={e => setFormData({...formData, repo_owner: e.target.value})} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700 uppercase ml-1">Repo Name</label>
                    <input placeholder="Repository" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.repo_name} onChange={e => setFormData({...formData, repo_name: e.target.value})} />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 uppercase ml-1">Personal Access Token</label>
                  <input type="password" placeholder="ghp_..." className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono" value={formData.github_token} onChange={e => setFormData({...formData, github_token: e.target.value})} />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-300">
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                  <div className="p-1.5 bg-purple-100 text-purple-600 rounded-lg"><Slack className="w-4 h-4" /></div><h3 className="text-sm font-bold text-slate-900">Slack Alerts</h3>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <input type="password" placeholder="Bot Token" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.slack_token} onChange={e => setFormData({...formData, slack_token: e.target.value})} />
                  <input placeholder="#alerts" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.slack_channel} onChange={e => setFormData({...formData, slack_channel: e.target.value})} />
                </div>
              </div>

              <div className="space-y-4">
                 <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                  <div className="p-1.5 bg-blue-100 text-blue-600 rounded-lg"><Terminal className="w-4 h-4" /></div><h3 className="text-sm font-bold text-slate-900">Jira Ticket Creation</h3>
                </div>
                <div className="space-y-4">
                  <input placeholder="Jira Domain" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.jira_url} onChange={e => setFormData({...formData, jira_url: e.target.value})} />
                  <div className="grid grid-cols-2 gap-4">
                    <input placeholder="Email" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.jira_email} onChange={e => setFormData({...formData, jira_email: e.target.value})} />
                    <input type="password" placeholder="API Token" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.jira_token} onChange={e => setFormData({...formData, jira_token: e.target.value})} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                 <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                  <div className="p-1.5 bg-gray-100 text-gray-600 rounded-lg"><ArrowUpRight className="w-4 h-4" /></div><h3 className="text-sm font-bold text-slate-900">Notion Docs</h3>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <input type="password" placeholder="Notion Token" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.notion_token} onChange={e => setFormData({...formData, notion_token: e.target.value})} />
                  <input placeholder="Database ID" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={formData.notion_database_id} onChange={e => setFormData({...formData, notion_database_id: e.target.value})} />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
              <div>
                ...
                <div className="relative mt-1">
                  <select className="w-full p-3.5 pl-4 pr-10 bg-slate-50 border border-slate-200 rounded-xl text-sm appearance-none" value={formData.manager_id} onChange={e => setFormData({...formData, manager_id: e.target.value})}>
                    <option value="">Select a Manager...</option>
                    {users.filter(u => u.role === 'manager').map(u => <option key={u._id} value={u._id}>{u.email}</option>)}
                  </select>
              ...
              <div>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden max-h-48 overflow-y-auto p-2 space-y-1 bg-white">
                  {users.filter(u => u.role === 'employee').map(u => (
                    <label key={u._id} className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer ${formData.employee_ids.includes(u._id) ? 'bg-indigo-50 border border-indigo-100' : 'hover:bg-slate-50 border border-transparent'}`}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${formData.employee_ids.includes(u._id) ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 bg-white'}`}>
                        {formData.employee_ids.includes(u._id) && <CheckCircle2 className="w-3 h-3 text-white" />}
                      </div>
                      <input type="checkbox" className="hidden" checked={formData.employee_ids.includes(u._id)} onChange={e => { const ids = e.target.checked ? [...formData.employee_ids, u._id] : formData.employee_ids.filter(id => id !== u._id); setFormData({...formData, employee_ids: ids}); }} /> 
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
          ...
          <Button className="bg-indigo-600 text-white px-8 rounded-xl" onClick={() => step < 3 ? setStep(step + 1) : handleLaunch()} disabled={loading}>
            {loading ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : (step === 3 ? 'Launch Project' : 'Continue')}
          </Button>
        </div>
      </motion.div>
    </div>
