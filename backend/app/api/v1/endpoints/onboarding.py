...
async def assign_team(
    ...
    # 1. Update Organization assignments - NOW SAVES FLAT FIELDS TOO
    await db["organizations"].update_one(
        {"_id": ObjectId(org_id)},
        {
            "$set": {
                f"assignments.{manager_id}": employee_ids,
                "manager_id": manager_id,       # <--- CRITICAL FIX for Webhook
                "employee_ids": employee_ids    # <--- CRITICAL FIX for Webhook
            },
            "$addToSet": {"members": {"$each": [manager_id] + employee_ids}}
        }
    )
