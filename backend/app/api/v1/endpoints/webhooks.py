...
async def get_org_credentials(org_id: str):
    """
    Fetches integration secrets for a specific organization.
    """
    try:
        db = get_database()
        query = {
...
async def send_slack_alert(creds, repo, pr_id, risk_data, notion_url, org_id):
    ...
    If Notion URL exists -> "View Report" button.
    If NOT -> "Create Report" button (carries org_id).
    """
    client = WebClient(token=creds["slack_token"])
    main_text = f"🚨 NovaScan Risk Alert: PR #{pr_id} in {repo}"
    ...
    if notion_url:
        ...
        action_elements.append({
            ...
            "style": "danger",
            "action_id": "trigger_notion_pm", 
            "value": f"{org_id}|https://github.com/{repo}/pull/{pr_id}" 
        })
...
# --- INTERACTIVE HANDLER (THE FIX) ---
@router.post("/slack/actions")
async def handle_slack_interactive(request: Request, background_tasks: BackgroundTasks):
    """
    Handles button clicks from Slack.
    Now correctly fetches credentials using the passed org_id.
    """
    try:
        form_data = await request.form()
        payload = json.loads(form_data["payload"])
        ...
        if action["action_id"] == "trigger_notion_pm":
            # 1. Extract Data from the Button Value
            # Value format: "org_id|pr_url"
            raw_value = action["value"]
            if "|" not in raw_value:
                logger.error("❌ Invalid Slack button value format")
                return {"status": "error"}
